<template>
  <el-dialog :visible.sync="modals" title="发送货" class="order_box" :before-close="handleClose" width="600px">
    <el-form ref="formItem" :model="formItem" label-width="110px" :rules="rules" @submit.native.prevent>
      ...
      <div v-if="formItem.type === '1'">
        <el-form-item label="发货类型：" prop="expressId">
          <el-radio-group v-model="formItem.expressRecordType" @change="changeRadio(formItem.expressRecordType)">
            ...
            <el-radio v-if="checkPermi(['admin:order:sheet:info'])" label="2">电子面单打印</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="快递公司：" prop="expressCode">
          ...
          >
            <el-option v-for="(item, i) in express" :key="i" :value="item.code" :label="item.name" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="formItem.expressRecordType === '1'" label="快递单号：" prop="expressNumber">
          <el-input v-model="formItem.expressNumber" placeholder="请输入快递单号" style="width:80%;" />
        </el-form-item>
        <template v-if="formItem.expressRecordType === '2'">
          ...
          <el-form-item label="寄件人姓名：" prop="toName">
            <el-input v-model="formItem.toName" placeholder="请输入寄件人姓名" style="width:80%;" />
          </el-form-item>
          <el-form-item label="寄件人电话：" prop="toTel">
            <el-input v-model="formItem.toTel" placeholder="请输入寄件人电话" style="width:80%;" />
          </el-form-item>
          <el-form-item label="寄件人地址：" prop="toAddr">
            <el-input v-model="formItem.toAddr" placeholder="请输入寄件人地址" style="width:80%;" />
          </el-form-item>
        </template>
      </div>
      <div v-if="formItem.type === '2'">
        <el-form-item label="送货人姓名：" prop="deliveryName">
          <el-input v-model="formItem.deliveryName" placeholder="请输入送货人姓名" style="width:80%;" />
        </el-form-item>
        <el-form-item label="送货人电话：" prop="deliveryTel">
          <el-input v-model="formItem.deliveryTel" placeholder="请输入送货人电话" style="width:80%;" />
        </el-form-item>
      </div>
      <div>
        <el-form-item label="">
