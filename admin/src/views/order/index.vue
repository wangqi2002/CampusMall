<template>
  <div class="divBox relative">
      ...
      <div class="clearfix">
        <div class="container">
          <el-form size="small" label-width="100px">
            <el-form-item v-if="checkPermi(['admin:order:status:num'])" label="订单状态：">
            ...
            <el-form-item class="width100">
              <el-button v-hasPermi="['admin:export:excel:order']" size="small" @click="exports">导出</el-button>
            </el-form-item>
          </el-form>
        </div>
      </div>
    ...
    <el-card class="box-card">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="right" :open-delay="800">
              ...
              <div v-if=" scope.row.productList && scope.row.productList.length" class="pup_card">
                <div v-for="(val, i ) in scope.row.productList" :key="i" class="tabBox acea-row row-middle" style="flex-wrap: inherit;">
                  <div class="">
                    <span class="tabBox_tit mr10">{{ val.info.productName + ' | ' }}{{ val.info.sku ? val.info.sku:'-' }}</span>
                    <span class="tabBox_pice">{{ '￥'+ val.info.price ? val.info.price + ' x '+ val.info.payNum : '-' }}</span>
              ...
              <div v-if="scope.row.refundStatus === 1 || scope.row.refundStatus === 2" class="refunding">
                <template>
                  <el-popover trigger="hover" placement="left" :open-delay="800">
                    <b slot="reference" style="color: #f124c7;">{{ scope.row.statusStr.value }}</b>
        ...
        <el-table-column label="操作" min-width="150" fixed="right" align="center">
          <template slot-scope="scope">
            <el-button v-if="scope.row.paid === false" v-hasPermi="['admin:order:update:price']" type="text" size="small" class="mr10" @click="edit(scope.row)">编辑</el-button>
            <el-button v-if="scope.row.statusStr.key === 'notShipped' && scope.row.refundStatus ===0" v-hasPermi="['admin:order:send']" type="text" size="small" class="mr10" @click="sendOrder(scope.row)">发送货</el-button>
            <el-button v-if=" scope.row.statusStr.key === 'toBeWrittenOff' && scope.row.paid == true && scope.row.refundStatus === 0 " v-hasPermi="['admin:order:write:update']" type="text" size="small" class="mr10" @click="onWriteOff(scope.row)">立即核销</el-button>
            <el-dropdown trigger="click">
              <span class="el-dropdown-link">
                更多<i class="el-icon-arrow-down el-icon--right" />
              </span>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-if="checkPermi(['admin:order:info'])" @click.native="onOrderDetails(scope.row.orderId)">订单详情</el-dropdown-item>
                <el-dropdown-item v-if="checkPermi(['admin:order:status:list'])" @click.native="onOrderLog(scope.row.orderId)">订单记录</el-dropdown-item>
                <el-dropdown-item v-if="checkPermi(['admin:order:mark'])" @click.native="onOrderMark(scope.row)">订单备注</el-dropdown-item>
    ...
    <!-- 发送货 -->
    <order-send ref="send" :order-id="orderId" @submitFail="getList" />

    <!-- 发送货视频号商品 -->
    <order-video-send ref="videoSend" :order-id="orderId" @submitFail="getList" />
