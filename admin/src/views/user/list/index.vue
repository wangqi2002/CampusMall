<template>
  <div class="divBox relative">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <el-tabs v-model="loginType" @tab-click="getList(1)">
          <el-tab-pane v-for="(item, index) in headeNum" :key="index" :label="item.name" :name="item.type.toString()" />
        </el-tabs>
        <div class="container">
          <el-form
            ref="userFrom"
            ...
            <el-row>
              ...
              <template v-if="collapse">
                ...
                <el-col :xs="24" :sm="24" :md="24" :lg="18" :xl="18">
                  ...
                  <el-col v-if="userFrom.country === 'CN'" v-bind="grid">
              ...
              <el-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6" class="text-right userFrom">
                <el-form-item>
                  <el-button
                    ...
                    @click="userSearchs"
                  >搜索</el-button>
                  <el-button class="ResetSearch mr10" size="small" @click="reset('userFrom')">重置</el-button>
      ...
      >
        ...
        <el-table-column prop="spreadNickname" label="推荐人" min-width="130" />
        ...
        <!--</template>-->
        <!--</el-table-column>-->
        <el-table-column prop="nowMoney" label="余额" min-width="100" />
          ...
          <template slot-scope="scope">
            ...
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  v-if="checkPermi(['admin:user:topdetail'])"
                  @click.native="onDetails(scope.row.uid)"
                ...
                <el-dropdown-item
                  v-if="checkPermi(['admin:user:group'])"
                  @click.native="setBatch('group', scope.row)"
                >设置分组</el-dropdown-item>
                <el-dropdown-item
                  v-if="checkPermi(['admin:user:tag'])"
                  @click.native="setBatch('label', scope.row)"
                >设置标签</el-dropdown-item>
                <el-dropdown-item
                  v-if="checkPermi(['admin:user:update:phone'])"
                  @click.native="setPhone(scope.row)"
    ...
    <el-dialog title="用户列表" :visible.sync="userVisible" width="900px">
      <user-list v-if="userVisible" @getTemplateRow="getTemplateRow" />
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="编辑" :visible.sync="visible" width="600px">
      <edit-from v-if="visible" :uid="uid" @resetForm="resetForm" />
    </el-dialog>
    <!--积分余额-->
    <el-dialog
      title="积分余额"
    ...
    <!--账户详情-->
    <el-dialog v-if="uid" title="用户详情" :visible.sync="Visible" width="1100px" :before-close="Close">
      <user-details v-if="Visible" ref="userDetails" :uid="uid" />
    </el-dialog>
    <!-- 用户等级 -->
    <el-dialog title="设置" :visible.sync="levelVisible" width="600px" :before-close="Close">
      <level-edit :level-info="levelInfo" :level-list="levelList" />
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 列表
    async getCityList() {
      const res = await logistics.cityListTree()
